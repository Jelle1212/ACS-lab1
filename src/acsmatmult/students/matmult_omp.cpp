...
Matrix<float> multiplyMatricesOMP(Matrix<float> a,
  ...
  for(i = 0; i < a.rows; i++){
    for(j = 0; j < matrix_BT.rows; j++){
      ...
      if(rest_count){
        avx rest_a, rest_b, prod2;
        rest_a.val = _mm256_setzero_ps();
        rest_b.val = _mm256_setzero_ps();
        ...
        if(rest_count == 1){
          ...
          rest_a.val = _mm256_maskload_ps(&A[i*a.rows+k], mask);
          rest_b.val = _mm256_maskload_ps(&B[j*b.rows+k], mask);
          prod2.val = _mm256_mul_ps(rest_a.val, rest_b.val); //dot product
          res += prod2.arr[0];
          // printf("REST1 val: %f\n", rest_a.arr[0]);
          // printf("REST2 val: %f\n", rest_b.arr[0]);
          // printf("SUM val: %f\n", prod2.arr[0]);
...
Matrix<double> multiplyMatricesOMP(Matrix<double> a,
  ...
  for(i = 0; i < a.rows; i++){
    for(j = 0; j < matrix_BT.rows; j++){
      ...
      if(rest_count){
        avxd rest_a, rest_b, prod2;
        rest_a.val = _mm256_setzero_pd();
        rest_b.val = _mm256_setzero_pd();
        prod2.val = _mm256_setzero_pd();
        if(rest_count == 1){
          __m256i mask = _mm256_set_epi64x(0x0000000000000000, 0x0000000000000000, 0x0000000000000000, 0xffffffffffffffff);
          rest_a.val = _mm256_maskload_pd(&A[i*a.rows+k], mask);
          rest_b.val = _mm256_maskload_pd(&B[j*b.rows+k], mask);
          prod2.val = _mm256_mul_pd(rest_a.val, rest_b.val); //dot product
          res += prod2.arr[0];
          // printf("REST1 val: %f\n", rest_a.arr[0]);
          // printf("REST2 val: %f\n", rest_b.arr[0]);
          // printf("SUM val: %f\n", prod2.arr[0]);
