...
void transposeMatrixSimdFloat(float *src, float *dst, uint32_t rows, uint32_t columns) {
...
void transposeMatrixSimdDouble(double *src, double *dst, uint32_t rows, uint32_t columns) {
  for(uint32_t i = 0; i < (rows * columns); i++){
    uint32_t j = i/rows;
    uint32_t k = i%rows;
    dst[i] = src[columns*k +j];
...
Matrix<float> multiplyMatricesSIMD(Matrix<float> a, Matrix<float> b) {
  ...
  transposeMatrixSimdFloat(&b[0], &matrix_BT[0], (uint32_t)b.rows, (uint32_t)b.columns);
...
Matrix<double> multiplyMatricesSIMD(Matrix<double> a,
  ...
  transposeMatrixSimdDouble(&b[0], &matrix_BT[0], (uint32_t)b.rows, (uint32_t)b.columns);
